            ...
            try:
                ...
                if pay_amount >= min_input_qiwi:
                    ...
                    if get_payments[4] == "form":
                        ...
                        send_message = f"<b>🆙 Пополнение баланса</b>\n" \
                                       f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n" \
                                       f"🥝 Для пополнения баланса, переведите нужную сумму на указанный кошёк или " \
...
async def check_qiwi_pay(call: CallbackQuery):
    ...
    get_payments = await get_paymentx()
    get_user_info = await get_userx(user_id=call.from_user.id)
    if get_payments[0] != "None" or get_payments[1] != "None" or get_payments[2] != "None":
        qiwi = QiwiP2P(get_payments[2])
        ...
        if pay_status == "PAID":
            get_purchase = await get_refillx("*", receipt=receipt)
            if get_purchase is None:

                await add_refillx(call.from_user.id, call.from_user.username, call.from_user.first_name, pay_comment,
                                  pay_amount,
    ...
    if get_payments[0] != "None" or get_payments[1] != "None" or get_payments[2] != "None":
        try:
            ...
            if pay_status:
                get_purchase = await get_refillx("*", receipt=receipt)
                if get_purchase is None:
                    await add_refillx(call.from_user.id, call.from_user.username, call.from_user.first_name, receipt,
                                      pay_amount,
                                      receipt, way_pay, datetime.datetime.today().replace(microsecond=0),
                                      int(time.time()))
